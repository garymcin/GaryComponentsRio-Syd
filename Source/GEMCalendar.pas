...
type
  ...
  TGEMCalendar = class(TCustomPanel)
    GrpPanel: TPanel;
    OkButton: TButton;
    CancelBtn: TButton;
...
constructor TGEMCalendar.Create(AOwner: TComponent);
begin
...
destructor TGEMCalendar.Destroy;
begin
  inherited;
end;


procedure TGEMCalendar.CreateWindowHandle(const Params: TCreateParams);
begin
...
procedure TGEMCalendar.CancelBtnClick(Sender: TObject);
...
procedure TGEMCalendar.OkButtonClick(Sender: TObject);
...
procedure TGEMCalendar.SetDate(const Value: TDate);
begin
  fDate := Value;
end;
