...
uses
  Windows, WinSpool, Messages, Classes, VCL.Graphics, VCL.Controls, SysUtils,
...
type
  ...
  private
    ...
    FOffset: {$IFDEF COMPILER4_UP} LongInt {$ELSE} DWORD {$ENDIF};
  ...
  protected
    ...
    property Offset: {$IFDEF COMPILER4_UP} LongInt {$ELSE} DWORD {$ENDIF} read FOffset;
...
uses
  ...
  {$IFDEF COMPILER4_UP} VCL.ImgList, {$ENDIF}
...
var
  Offsets: TIntegerList;
  Entry: TMetafileEntry;
  HeaderOffset: {$IFDEF COMPILER4_UP} LongInt {$ELSE} DWORD {$ENDIF};
  BaseOffset: {$IFDEF COMPILER4_UP} LongInt {$ELSE} DWORD {$ENDIF};
  DataSize: LongInt;
  I: Integer;
begin
